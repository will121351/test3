...
<div class="content-header">
    <h1><s:message code="visitLog.countryAnalysis" />
        ...
        <div class="box-body table-responsive">
            <form class="form-inline ls-search" action="country_analysis.do" method="get">
                <div id="radio" class="form-group">
                    <input type="radio" id="radioLast30Day" onclick="location.href='country_analysis.do?period=last30Day';"<c:if test="${empty period || period eq 'last30Day'}"> checked="checked"</c:if>>
                    <label for="radioLast30Day"><s:message code="visitLog.trafficAnalysis.last30Day" /></label>
                    <input type="radio" id="radioLast1Year" onclick="location.href='country_analysis.do?period=last1Year';"<c:if test="${period eq 'last1Year'}"> checked="checked"</c:if>>
                    <label for="radioLast1Year"><s:message code="visitLog.trafficAnalysis.last1Year" /></label>
                    <input type="radio" id="radioLast3Year" onclick="location.href='country_analysis.do?period=last3Year';"<c:if test="${period eq 'last3Year'}"> checked="checked"</c:if>>
                ...
                var option = {
                    ...
                    legend: {
                        ...
                        data: [
                            ...
                            <c:when test="${'LAN' eq bean.name}">'<s:message code="visitLog.LAN"/>'</c:when>
                    ...
                    series: [
                        {
                            ...
                            data: [
                                ...
                                {
                                    value:${bean.views},
                                    name: '<c:choose><c:when test="${'UNKNOWN' eq bean.name}"><s:message code="visitLog.UNKNOWN"/></c:when><c:when test="${'LAN' eq bean.name}"><s:message code="visitLog.LAN"/></c:when><c:otherwise><c:out value="${bean.name}"/></c:otherwise></c:choose>'
                                }<c:if test="${!status.last}">, </c:if>
                                </c:when>
                                <c:otherwise>
                                <c:set var="count" value="${count+bean.views}"/>
                                </c:otherwise>
                                </c:choose>
                                </c:forEach>
